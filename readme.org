...
#+begin_src emacs-lisp
  (use-package apheleia
    :defer t
    :config
    ;; Define a formatter named `eglot` that uses apheleia's built-in Eglot integration.
    (setf (alist-get 'eglot apheleia-formatters) '(eglot))
...
*** treesitter context
    #+begin_src emacs-lisp
      ;; (add-to-list 'load-path "~/projects/contrib/treesitter-context.el")
