...
** COMMENT hide all stars
   from [[https://www.reddit.com/r/emacs/comments/9wukv8/comment/e9pkggv][this]] reddit comment
   #+begin_src emacs-lisp
     ;; remove everything
...
#+begin_src emacs-lisp
  (use-package treesit-fold
    ...
    :hook (treesit-mode . treesit-fold-mode)
    :config
...
#+begin_src emacs-lisp
  (use-package auto-dark
    ...
    (auto-dark-themes '((kaolin-dark) (kaolin-light)))
  ...
  (defun my-centaur-tabs-hide-tab (x)
    ...
    (let ((name (format "%s" x)))
      ...
      (or
       ...
       (string-suffix-p "*epc" name)
       (string-suffix-p "*helm" name)
       (string-suffix-p "*Helm" name)
       (string-suffix-p "*Compile-Log*" name)
       (string-suffix-p "*lsp" name)
       (string-suffix-p "*company" name)
       (string-suffix-p "*Flycheck" name)
       (string-suffix-p "*tramp" name)
       (string-suffix-p " *Mini" name)
       (string-suffix-p "*help" name)
       (string-suffix-p "*straight" name)
       (string-suffix-p " *temp" name)
       (string-suffix-p "*Help" name)
       (string-suffix-p "*mybuf" name)
...
#+begin_src emacs-lisp
  (use-package centaur-tabs
    :demand
    :config
    (centaur-tabs-change-fonts "Margem Rounded Test" 120)
    (centaur-tabs-mode t)
    :bind
    (:map centaur-tabs-mode-map
          (("C-c t j" . centaur-tabs-ace-jump)
           ("C-c t RET" . centaur-tabs-local-mode)
...
*** treemacs extensions
    #+begin_src emacs-lisp
      ...
      (use-package lsp-treemacs
        ...
        (define-key lsp-mode-map (kbd "C-c l g s") 'toggle-lsp-treemacs-modes)
        
        ;; :bind (:map lsp-mode-map
        ;;             ("C-c l g s" . toggle-lsp-treemacs-modes))
...
#+begin_src emacs-lisp
  (use-package vertico-posframe
    :config
    ;; https://github.com/tumashu/vertico-posframe/issues/27#issuecomment-1423156044
    (defun vertico-posframe-hide-minibuffer (BUFFER WINDOW-POINT)
    ...
    (advice-add 'vertico-posframe--show :after 'vertico-posframe-hide-minibuffer)

    (setq vertico-posframe-width 114)
    (vertico-posframe-mode))
#+end_src
** custom set faces
   #+begin_src emacs-lisp
     (custom-set-faces
      '(mode-line ((t (:inherit 'default :family "Conta" :height 120 :box nil))))
      '(mode-line-active ((t (:inherit mode-line :box nil))))
      '(mode-line-inactive ((t (:inherit mode-line-active :box nil))))
      '(doom-modeline-buffer-modified ((t (:inherit (warning)))))

      '(blamer-face ((t :family "Dank Mono" :height 100 :italic t)))
      ...
      '(font-lock-comment-face ((t :font "Dank Mono" :slant italic :height 110))))
     ...
     (dolist (face '((org-level-1 . 1.4)
                     ...
                     (org-level-7 . 1.0)
                     (org-level-8 . 1.0)))
       (set-face-attribute (car face) nil :font "Margem Condensed Test" :weight 'medium :height (cdr face)))
...
** custom theme set faces
   #+begin_src emacs-lisp
     ...
     (custom-theme-set-faces
      'user
      '(fixed-pitch ((t (:family "VictorMono Nerd Font" :height 120))))
      '(variable-pitch ((t (:family "Margem Test" :height 130)))) ;; short stack
